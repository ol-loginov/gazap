Waypalm.extendNamespace('Ui', function (N, G) {
    ...
    N.PlainMap = function (opts) {
        ...
        this.$domRoot = H.create('div').appendTo(H.byID(opts.container))
            ...
            .addClass('waypalm-ui-map-container');
        ...
        this.$domLayers = H.create('div').appendTo(this.$domRoot)
            .addClass('waypalm-ui-map-layers')
            .style({position:'absolute', left:0, top:0, width:0, height:0, overflow:'visible'});
        this.layers = [];
    ...
    N.PlainMap.prototype = {
        ...
        createLayer:function (name) {
            ...
            layer.$node = H.create('div').appendTo(this.$domLayers)
                .addClass('waypalm-ui-map-child-layer')
                .style({position:'absolute', left:0, top:0, width:0, height:0, overflow:'visible'});
        ...
        refreshTiles:function () {
            ...
            for (var x = boundLeft; x <= boundRight; x += q) {
                for (var y = boundBottom; y <= boundTop; y += q) {
                    ...
                    request.src = this.tileServer + Waypalm.format('/tiles/m{0}/c{1}s{2}x{3}y{4}', this.tileMap, request.scale, request.size, request.x, request.y);
                    this.trigger('tile.request', request);
        ...
        refreshTile:function (req) {
            var id = Waypalm.format('waypalm-ui-map-tile-{0}d{1}d{2}d{3}', req.scale, req.size, req.x, req.y);
            var img = H.byID(id);
            if (img == null) {
                img = H.create('img')
                    .attr('id', id)
